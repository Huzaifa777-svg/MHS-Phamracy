...
class medicine():
  def __init__(self, root):
     self.root = root
     self.root.title("MHS Pharmacy Management System")

     self.width = self.root.winfo_screenwidth()
     self.height = self.root.winfo_screenheight()
     self.root.geometry(f"{self.width}x{self.height}+0+0")

     title = tk.Label(self.root,text="MHS Pharmacy Management Sysytem",bd=4,relief="solid",fg="black",bg=self.clr(160,160,160), font = ("Arial",50,"bold")) 
     ...
     self.medIn.grid(row=0,column=1,padx=10)
     ...
     self.quanIn=tk.Entry(addFrame,width=20,bd=2,font=("Arial",15))
     self.quanIn.grid(row=2,column=1,padx=10,pady=30)

     expLbl=tk.Label(addFrame,bg=self.clr(150,220,150),text="Expiry: ",font=("Arial",15,"bold"))
     expLbl.grid(row=3,column=0,padx=10,pady=30)
     self.expIn=tk.Entry(addFrame,width=20,bd=2,font=("Arial",15))
     self.expIn.grid(row=3,column=1,padx=10,pady=30)

     addBtn = tk.Button(addFrame,command=self.addFun,text="Add Medicine",width=20,font=("Arial",15,"bold"),bd=2,relief="raised")
     ...
     btnFrame.place(width=self.width/4-100,height=self.height-180,x = self.width-390,y=100) 

     srchBtn = tk.Button(btnFrame,command=self.searchFun,text="Search",width=12,bd=3,relief="raised",font=("Arial",15,"bold"))
     srchBtn.grid(row=0,column=0,padx=55,pady=40)

     saleBtn = tk.Button(btnFrame,command=self.saleFun,text="Sale",width=12,bd=3,relief="raised",font=("Arial",15,"bold"))
     saleBtn.grid(row=1,column=0,padx=55,pady=40)

     upBtn = tk.Button(btnFrame,command=self.updFun,text="Update",width=12,bd=3,relief="raised",font=("Arial",15,"bold"))
     upBtn.grid(row=2,column=0,padx=55,pady=40)

     allBtn = tk.Button(btnFrame,text="Show_All",command=self.showAll,width=12,bd=3,relief="raised",font=("Arial",15,"bold"))
     allBtn.grid(row=3,column=0,padx=55,pady=40)
  ...
  def addFun(self):
    ...
    if med and p and q and exp:
      ...
      try:
        self.dbFun()
        self.cur.execute("insert into medics(medicine,price,quant,exp) value(%s,%s,%s,%s)" ,
                         (med,price,quant,exp))
        self.con.commit()
  ...
  def dbFun(self):
    self.con = pymysql.connect(host="localhost",user="root",passwd="Sedil@1234",database="rec")
    self.cur = self.con.cursor()
  def searchFun(self):
    self.srchFrame = tk.Frame(self.root,bd=5,relief="ridge",bg="light gray")
    self.srchFrame.place(width=self.width/3,height=self.height-400,x=self.width-940,y=100) 
  ...
  def saleFun(self):
    ...
    self.saleFrame.place(width=self.width/3,height=self.height-400,x=self.width-940,y=100) 
    ...
    self.seleQuant = tk.Entry(self.saleFrame,width=20,bd=2,font=("Arial",15,"bold"))
    self.seleQuant.grid(row=1,column=1)
  ...
  def updFun(self):
    ...
    self.updFrame.place(width=self.width/3,height=self.height-400,x=self.width-940,y=100) 
